...
<html>
    <head>
       ...
       <script type="text/javascript" src="calendarDateInput.js"></script>
        ...
        var sqlQuery = document.sqlForm.sqlQuery.value;
        if (sqlQuery.trim() === "") {
            alert("Please enter a valid SQL query.");
            return;
        }

        // Perform an AJAX request to execute the SQL query
        var xhr = new XMLHttpRequest();
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4 && xhr.status == 200) {
                document.getElementById("sqlResult").innerHTML = xhr.responseText;
            }
        };

        // Replace the URL with the appropriate servlet or JSP that handles the SQL execution
        xhr.open("GET", "executeSQL.jsp?sqlQuery=" + encodeURIComponent(sqlQuery), true);
        xhr.send();
        }       
...
<h4>Post Emergency Requirement</h4>
	...
	<div id="footer">Blood Donor Symbiosis.</div>
         <% 
}
    catch (Exception e) {
            e.printStackTrace();
     ...
     %>
        ...
        <form name="sqlForm">
        <textarea name="sqlQuery" rows="4" cols="50" placeholder="Enter SQL Query"></textarea><br>
        <input type="button" value="Execute Query" size="4" name="b2" onClick="executeSQLQuery()"/>
        <br><br>
